...
class LearningRateMultiplier(Optimizer):
    ...
    # Arguments
        ...
        **kwargs: The arguments for instantiating the wrapped optimizer
            class.
    """
    def __init__(self, optimizer, lr_multipliers=None, decay_multipliers=None, **kwargs):
        self._class = optimizer
        self._optimizer = optimizer(**kwargs)
        self._lr_multipliers = lr_multipliers or {}
        self._decay_multipliers = decay_multipliers or {}
    ...
    def _get_multiplier(self, param):
        ...
        for k in self._decay_multipliers.keys():
            if k in param.name:
                return self._decay_multipliers[k]
    ...
    def get_updates(self, loss, params):
        ...
        updates = []
        base_decay = self._optimizer.decay
        for param, multiplier in mult_decay_params.items():
            self._optimizer.decay = base_decay * multiplier
            updates.extend(self._optimizer.get_updates(loss, [param]))
    ...
    def get_config(self):
        config = {'optimizer': self._class,
                  ...
                  'decay_multipliers': self._decay_multipliers}
...
# class DecayMultiplier(Optimizer):
#     """Optimizer wrapper for per layer learning rate.
#     This wrapper is used to add per layer learning rates by
#     providing per layer factors which are multiplied with the
#     learning rate of the optimizer.
...
#             class.
#     """
#     def __init__(self, optimizer, decay_multipliers=None, **kwargs):
#         self._class = optimizer
#         self._optimizer = optimizer(**kwargs)
#         self._decay_multipliers = decay_multipliers or {}
...
#     def __setattr__(self, name, value):
#         if name.startswith('_'):
#             super(DecayMultiplier, self).__setattr__(name, value)
#         else:
#             self._optimizer.__setattr__(name, value)


# get_custom_objects().update({'DecayMultiplier': DecayMultiplier})
